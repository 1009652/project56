...
def getShips():
    ...
    for msg in UDPStream(UDP_IP, UDP_PORT):
        ...
        # for element in keys:
        #     ships.append(element + ",")
        # print(ships)
        # print(f"Decoded message: {decoded_data}")
        if decoded_msg.content['type'] == 5 or decoded_msg.content['type'] == 8:
            print(f"WRONG AIS TYPE MESSAGE{decoded_msg.content['type']}")
            # print(decoded_msg.content)
        else:
            try:
                selected_data = [decoded_msg.content[key] for key in keys]
