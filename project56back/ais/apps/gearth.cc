// Compile with g++ -o gearth gearth.cc -lmysqlclient
#include <cstdio>
#include <string.h>
#include <cstdlib>
#include <time.h>
...
void lagkml()
{
	...
	sqlquery << "select distinct mmsi from ais_position where timediff(now(),time) < 120";
	//sqlquery << (int)nu-120;
	...
	sqlquery.str("");
	sqlquery << "select distinct mmsi from ais_position where timediff(now(),time) < 1800";
	//sqlquery << (int)nu-600;
	sqlquery << " and timediff(now(),time) >= 120 ";
	//sqlquery << (int)nu-120;
	sqlquery << " order by mmsi";
	//printf("%s\n",sqlquery.str().c_str());
	res = query(&conn,sqlquery.str().c_str());
	...
	//	printf("%d:",mmsier[i]);
		...
		posteller = 0;
		sqlquery.str("");
		sqlquery << "select name,A,B,C,D,draught from ais_vesseldata where A != 0 and mmsi = ";
		sqlquery << mmsier[i];
...
//				printf("%s\n",row[1]);
		...
		sqlquery.str("");
		sqlquery << "select destination from ais_vesseldata where mmsi = ";
		sqlquery << mmsier[i];
		sqlquery << " and timediff(now(),time) < 3600";
	...
	//		printf(" - Destination: %s ",row[0]);
		...
		mysql_free_result(res);
		sqlquery.str("");
		sqlquery << "select time,course,heading,speed,longitude,latitude from ais_position where longitude != 0 and mmsi = ";
		sqlquery << mmsier[i];
		sqlquery << " order by time DESC limit 1";
	...
	//	printf("\n");
		if ((row = mysql_fetch_row(res)) != NULL)
		{
			...
			status = 0;//atoi(row[6]);
			course3 = atoi(row[2]);
			if(course3 == 511) course = course2;
			else course = (float)course3;
		}
		else kmlfil << "\n";
		...
		sqlquery.str("");
		sqlquery << "select heading,time,longitude,latitude, course,speed from ais_position where mmsi = ";
		sqlquery << mmsier[i];
		sqlquery << " and timediff(now(),time) < (3600*5)";
